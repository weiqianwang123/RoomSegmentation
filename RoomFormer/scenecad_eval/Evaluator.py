...
class Evaluator_SceneCAD():
    ...
    def get_quantitative(self, gt_polys, ignore_mask_region, pred_polys=None, masks_list=None, img_size=(256, 256), dataset_type="s3d"):
        ...
        for gt_ind, gt_map in enumerate(gt_room_map_list):
            ...
            #     plt.imshow(gt_map)
            #     plt.show()
        fp = len(pred_room_map_list) - tp

        precision = tp / (tp + fp) if (tp + fp) > 0 else 0
        recall = tp / (tp + fn) if (tp + fn) > 0 else 0
        average_iou = average_iou/len(gt_room_map_list)
        ap50 = precision * recall
        print(ap50)
        # save_ap_value(ap50, 'eval_results.json')
...
# 定义一个函数来保存单个AP值
def save_ap_value(ap_value, filename):
    # 如果文件存在，加载现有数据
